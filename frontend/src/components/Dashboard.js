  ...
  const handleDeleteStudent = async (id) => {
    if(window.confirm("⚠️ WARNING: Delete permanently?")) {
        try {
            ...
            alert("🗑️ Deleted!");
            setViewStudent(null); // Popup band karein
            loadData(); 
                    ...
                    <tr key={std._id}>
                        ...
                        <td style={styles.tableCell}>
                            {!std.approval.isApproved ? (
                                ...
                                <button onClick={()=>setViewStudent(std)} style={{...styles.btnAction, background:"#dcfce7", color:"green"}}>View 👁️</button>
                            )}
                        </td>
                    </tr>
                ))}
        ...
        <div style={{position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", display:"flex", justifyContent:"center", alignItems:"center", zIndex:1000}}>
            <div style={{background:"white", padding:"25px", borderRadius:"20px", width:"90%", maxWidth:"500px", maxHeight:"90vh", overflowY:"auto"}}>
                ...
                <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", borderBottom:"1px solid #eee", paddingBottom:"10px"}}>
                    <h2 style={{margin:0, color: "#2c3e50"}}>👤 {viewStudent.name}</h2>
                ...
                {/* Details Grid */}
                <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px", marginBottom:"20px", fontSize:"14px", color: "#334155"}}>
                    <div><small style={{color:"gray", fontWeight:"bold"}}>FATHER</small><div>{viewStudent.fatherName}</div></div>
                    <div><small style={{color:"gray", fontWeight:"bold"}}>MOBILE</small><div>{viewStudent.mobile}</div></div>
                    <div><small style={{color:"gray", fontWeight:"bold"}}>COURSE</small><div>{viewStudent.course}</div></div>
                    <div><small style={{color:"gray", fontWeight:"bold"}}>JOINED</small><div>{new Date(viewStudent.admissionDate).toLocaleDateString()}</div></div>
                </div>
                ...
                {/* Payment History */}
                <h4 style={{margin:"0 0 10px", color: "#1e293b"}}>📜 History</h4>
                <div style={{marginBottom: "20px", maxHeight: "150px", overflowY: "auto", border: "1px solid #eee", borderRadius: "8px"}}>
                    {viewStudent.paymentHistory.length > 0 ? viewStudent.paymentHistory.map((p,i) => (
                        <div key={i} style={{display:"flex", justifyContent:"space-between", padding:"10px", borderBottom:"1px solid #eee", fontSize:"13px", background: "#fff"}}>
                            <span>{new Date(p.date).toLocaleDateString()}</span>
                            <span style={{fontWeight:"bold"}}>₹{p.amount}</span>
                        </div>
                    )) : <div style={{padding:"10px", textAlign:"center", color:"gray"}}>No payments yet.</div>}
                </div>

                {/* 👇 ACTION BUTTONS (Govt Status, Edit, Delete) */}
                <div style={{borderTop:"1px solid #eee", paddingTop:"15px", display:"flex", flexDirection: "column", gap:"10px"}}>
                    ...
                    <div style={{display: "flex", gap: "10px"}}>
                        ...
                        <button onClick={() => alert("Edit feature coming soon!")} style={{flex: 1, padding:"10px", background:"#3b82f6", color: "white", border:"none", borderRadius:"8px", cursor:"pointer", fontWeight:"bold"}}>✏️ Edit</button>
